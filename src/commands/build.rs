...
async fn build_vehicles(cli: &Cli) {
    ...
    db.create_vehicle_table().await.expect("Failed to create vehicle table");
    db.insert_vehicles(vehicles).await.expect("Failed to insert vehicle records");
}
...
async fn build_signals(cli: &Cli) {
    ...
    db.create_signal_table().await.expect("Failed to create signal table");
    ...
    db.create_signal_indexes().await.expect("Failed to create signal indexes");
}
